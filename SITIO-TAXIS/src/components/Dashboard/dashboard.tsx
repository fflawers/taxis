...
// Definimos la estructura para que TypeScript no marque error en 'datos'
...
function Dashboard() {
  ...
  // Función siguiendo tu estructura de fetchAcuerdos
  const fetchAnalisis = async () => {
    try {
      // Usamos (import.meta as any) para evitar el error de TS en Vite
      const baseUrl = (import.meta as any).env.VITE_API_URL;
      const res = await fetch(`${baseUrl}/dashboard/analisis/${modulo}`);
  ...
  return (
    <div>
      ...
      <div className="container mt-4 mb-5">
        ...
        {/* Selector de Módulos (Lista Desplegable) */}
              ...
              <thead className="table-dark">
                <tr>
                  {modulo === "resumen_30_dias" && (
                    <><th>Total Reportes</th><th>Taxistas Activos</th><th>Total Taxis</th></>
                  )}
                  {(modulo === "ingresos_top" || modulo === "choferes_reportados" || modulo === "viajes_top") && (
                    <><th>Posición</th><th>Nombre del Chofer</th><th>{modulo === "ingresos_top" ? "Monto Total" : modulo === "viajes_top" ? "Total Viajes" : "Total Reportes"}</th></>
                  )}
                </tr>
              </thead>
              <tbody>
                {datos.length > 0 ? (
                      ...
                      {(modulo === "ingresos_top" || modulo === "choferes_reportados" || modulo === "viajes_top") && (
                        <>
                          ...
                          <td className="fw-bold text-primary">
                            {modulo === "ingresos_top" ? `$${item.total_ingreso}` : modulo === "viajes_top" ? item.total_viajes : item.total_reportes}
                          </td>
                        </>
                      )}
                    </tr>
                  ...
                  <tr><td colSpan={3}>Cargando datos o no hay registros en los últimos 30 días...</td></tr>
                )}
              </tbody>
            </table>
          </div>
